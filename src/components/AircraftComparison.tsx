...
export default function AircraftComparison({ isOpen, onClose, initialAircraft, lang: inputLang }: AircraftComparisonProps) {
  ...
  const renderSpecRow = (
    ...
    return (
      <div className="border-b border-white/5 py-3 md:py-4 hover:bg-slate-900/10 transition-colors px-3 md:px-4 rounded-xl">
        <div className="flex items-center gap-1.5 mb-2">
          <div className="p-1 bg-slate-800/80 rounded text-sky-400 select-none">{icon}</div>
          <span className="text-[10px] md:text-[11px] uppercase font-bold text-slate-400 tracking-wider font-mono">{label}</span>
        </div>
            ...
            {v1 !== null ? (
              <div className="space-y-1">
                <div className="text-xs md:text-sm font-bold text-white">
                  ...
                  {v1 !== '' && <span className="text-[9px] md:text-[10px] text-slate-500 font-sans font-medium">{unit}</span>}
                </div>
                {comp1 && (
                  <div className={`text-[9px] md:text-[10px] py-0.5 px-2 inline-block rounded-full border font-bold ${
                    isSlot1Higher ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/10' : 
                    comp1 === text.same ? 'bg-slate-800/40 text-slate-400 border-slate-700/50' :
                ...
                {isNumeric && typeof v1 === 'number' && typeof v2 === 'number' && (
                  <div className="w-full h-1 bg-slate-950 rounded-full mt-1 overflow-hidden">
                    <div style={{ width: barWidth1 }} className={`h-full rounded-full transition-all duration-500 ${isSlot1Higher ? 'bg-emerald-500' : 'bg-slate-600'}`} />
                  </div>
                )}
              </div>
            ) : (
            ...
            {v2 !== null ? (
              <div className="space-y-1">
                <div className="text-xs md:text-sm font-bold text-white">
                  ...
                  {v2 !== '' && <span className="text-[9px] md:text-[10px] text-slate-500 font-sans font-medium">{unit}</span>}
                </div>
                {comp2 && (
                  <div className={`text-[9px] md:text-[10px] py-0.5 px-2 inline-block rounded-full border font-bold ${
                    isSlot2Higher ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/10' : 
                    comp2 === text.same ? 'bg-slate-800/40 text-slate-400 border-slate-700/50' :
                    'bg-slate-950 text-slate-400 border-white/5'
                  }`}>
                    {comp2}
                  </div>
                ...
                {isNumeric && typeof v2 === 'number' && typeof v1 === 'number' && (
                  <div className="w-full h-1 bg-slate-950 rounded-full mt-1 overflow-hidden flex justify-end">
                    <div style={{ width: barWidth2 }} className={`h-full rounded-full transition-all duration-500 ${isSlot2Higher ? 'bg-emerald-500' : 'bg-slate-600'}`} />
                  </div>
                )}
              </div>
            ) : (
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/95 backdrop-blur-md flex flex-col justify-start md:justify-center items-center p-3 md:p-6 select-none font-sans">
      
      <div className="w-full max-w-6xl bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border border-white/5 shadow-2xl rounded-2xl md:rounded-3xl overflow-hidden flex flex-col my-auto relative z-10 shrink-0 max-h-[92vh] md:max-h-[90vh]">
        
        {/* MODAL HEADER */}
        <div className="p-3.5 md:p-5 border-b border-white/5 flex items-center justify-between bg-slate-950/40 shrink-0">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              <h2 className="text-sm md:text-base font-extrabold text-white tracking-tight flex items-center gap-1.5">
              ...
              <p className="text-[9px] md:text-[10px] text-slate-500 font-mono">{text.subtitle}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-1.5 md:p-2 bg-white/5 hover:bg-white/10 rounded-xl text-slate-400 hover:text-white transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 border-b border-white/5 divide-x divide-white/5 bg-slate-950/20 shrink-0">
          
          {/* SLOT 1 SELECTION */}
          <div className="p-2.5 md:p-4 relative">
            {slot1 ? (
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 bg-slate-900/50 p-2 md:p-3 rounded-xl md:rounded-2xl border border-white/5">
                <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
                  <div className="shrink-0 scale-90 md:scale-105">
                    {getFlag(slot1.country)}
                  </div>
                  <div className="overflow-hidden">
                    <h4 className="text-[11px] md:text-sm font-bold text-white truncate">{slot1.name}</h4>
                    <p className="text-[8px] md:text-[10px] text-slate-500 font-mono truncate">{slot1.manufacturer} • {translateCategory(slot1.category, currentLang)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1 md:gap-1.5 self-end xl:self-auto shrink-0 mt-1 xl:mt-0">
                  <button 
                    ...
                    className="p-1 px-2 bg-sky-500/10 hover:bg-sky-500/15 border border-sky-500/20 text-sky-400 font-bold text-[9px] md:text-[10px] rounded-lg transition-all cursor-pointer"
                  >
                    {text.change}
                  </button>
                  <button 
                    onClick={() => { setSlot1(null); setSearchOpen1(false); }}
                    className="p-1 text-[9px] md:text-[10px] hover:text-rose-400 font-bold text-slate-500 transition-colors cursor-pointer"
                  >
              ...
              <button
                onClick={() => { setSearchOpen1(!searchOpen1); setSearchOpen2(false); }}
                className="w-full flex items-center justify-center gap-1.5 p-2.5 md:p-3.5 bg-slate-950/50 hover:bg-slate-950 border border-dashed border-slate-800 hover:border-sky-500/50 text-slate-450 hover:text-sky-400 rounded-xl md:rounded-2xl transition-all duration-300 font-mono text-[10px] md:text-xs font-semibold cursor-pointer"
              >
            ...
            <AnimatePresence>
              {searchOpen1 && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute left-2.5 right-2.5 md:left-4 md:right-4 top-[calc(100%+4px)] bg-slate-900 border border-slate-800 rounded-xl md:rounded-2xl shadow-xl z-50 p-2 md:p-3 max-h-[250px] md:max-h-[300px] flex flex-col"
                >
                  <div className="relative mb-2 shrink-0">
                    <Search className="absolute left-2.5 top-2 w-3 h-3 text-slate-500" />
                    <input
                      type="text"
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg py-1.5 pl-7 pr-3 text-[11px] outline-none text-white placeholder-slate-500 focus:border-sky-500"
                    ...
                    {filtered1.length > 0 ? (
                      filtered1.map(aircraft => (
                        <button
                          ...
                          className={`w-full text-left p-1.5 md:p-2 hover:bg-slate-800 rounded-lg text-[11px] flex justify-between items-center transition-colors ${slot1?.id === aircraft.id ? 'bg-sky-500/10 text-sky-400 font-bold' : 'text-slate-300'}`}
                        >
                          <span className="truncate pr-2">{aircraft.name}</span>
                          <span className="text-[8px] md:text-[9px] text-slate-500 font-mono uppercase bg-slate-950/40 px-1.5 py-0.5 rounded border border-white/5 shrink-0">{aircraft.manufacturer}</span>
                        </button>
                      ))
                    ) : (
            ...
            {slot2 ? (
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 bg-slate-900/50 p-2 md:p-3 rounded-xl md:rounded-2xl border border-white/5">
                <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
                  ...
                  <div className="overflow-hidden">
                    <h4 className="text-[11px] md:text-sm font-bold text-white truncate">{slot2.name}</h4>
                    <p className="text-[8px] md:text-[10px] text-slate-500 font-mono truncate">{slot2.manufacturer} • {translateCategory(slot2.category, currentLang)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1 md:gap-1.5 self-end xl:self-auto shrink-0 mt-1 xl:mt-0">
                  <button 
                    onClick={() => { setSearchOpen2(!searchOpen2); setSearchOpen1(false); }}
                    className="p-1 px-2 bg-[#818cf8]/10 hover:bg-[#818cf8]/15 border border-[#818cf8]/20 text-[#818cf8] font-bold text-[9px] md:text-[10px] rounded-lg transition-all cursor-pointer"
                  >
                    {text.change}
                  </button>
                  <button 
              ...
              <button
                onClick={() => { setSearchOpen2(!searchOpen2); setSearchOpen1(false); }}
                className="w-full flex items-center justify-center gap-1.5 p-2.5 md:p-3.5 bg-slate-950/50 hover:bg-slate-950 border border-dashed border-slate-800 hover:border-indigo-500/50 text-slate-450 hover:text-indigo-405 rounded-xl md:rounded-2xl transition-all duration-300 font-mono text-[10px] md:text-xs font-semibold cursor-pointer"
              >
                <Search className="w-3 h-3 md:w-3.5 md:h-3.5 text-slate-500" />
                {text.select}
              </button>
              ...
              {searchOpen2 && (
                ...
                >
                  <div className="relative mb-2 shrink-0">
                    ...
                    <input
                      type="text"
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg py-1.5 pl-7 pr-3 text-[11px] outline-none text-white placeholder-slate-500 focus:border-[#818cf8]"
                      placeholder={text.searchPlaceholder}
                      value={query2}
                      onChange={(e) => setQuery2(e.target.value)}
                    />
                    {query2 && (
                      <button onClick={() => setQuery2('')} className="absolute right-2.5 top-1.5 text-slate-400 hover:text-white text-[9px] font-bold">{text.clear}</button>
                    )}
                  </div>
                  <div className="overflow-y-auto flex-1 space-y-1 scrollbar-thin">
                    {filtered2.length > 0 ? (
                      filtered2.map(aircraft => (
                        <button
                          ...
                          className={`w-full text-left p-1.5 md:p-2 hover:bg-slate-800 rounded-lg text-[11px] flex justify-between items-center transition-colors ${slot2?.id === aircraft.id ? 'bg-[#818cf8]/10 text-[#818cf8] font-bold' : 'text-slate-300'}`}
                        >
                          <span className="truncate pr-2">{aircraft.name}</span>
                          <span className="text-[8px] md:text-[9px] text-slate-500 font-mono uppercase bg-slate-950/40 px-1.5 py-0.5 rounded border border-white/5 shrink-0">{aircraft.manufacturer}</span>
                        </button>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth p-3.5 md:p-6 space-y-4 max-h-[calc(92vh-190px)]">
          ...
          {(!slot1 && !slot2) ? (
            <div className="py-16 md:py-24 text-center">
              <ArrowLeftRight className="w-10 h-10 md:w-12 md:h-12 stroke-[1.5] text-slate-600 mx-auto opacity-50 mb-3 animate-bounce" />
              <h3 className="text-xs md:text-sm font-bold text-slate-300">{text.chooseTwoTitle}</h3>
              <p className="text-[10px] md:text-xs text-slate-500 mt-1 max-w-sm mx-auto font-medium leading-relaxed px-4 text-center">
                {text.chooseTwoDesc}
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="bg-slate-950/30 border border-white/5 rounded-xl md:rounded-2xl overflow-hidden divide-y divide-white/5">
              ...
              {slot1 && slot2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="bg-sky-500/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-sky-500/10">
                    <h5 className="text-[9px] md:text-[10px] font-bold font-mono tracking-wide text-sky-450 uppercase mb-1 flex items-center gap-1.5">
                      <Info className="w-3.5 h-3.5" /> {text.uniquenessLabel}: {slot1.name}
                    </h5>
                    <p className="text-[11px] md:text-xs text-slate-300 leading-relaxed font-sans">{translateText(slot1.uniqueness, currentLang, true)}</p>
                  </div>
                  <div className="bg-[#818cf8]/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-[#818cf8]/10">
                    <h5 className="text-[9px] md:text-[10px] font-bold font-mono tracking-wide text-[#818cf8] uppercase mb-1 flex items-center gap-1.5">
                      <Info className="w-3.5 h-3.5" /> {text.uniquenessLabel}: {slot2.name}
                    </h5>
                    <p className="text-[11px] md:text-xs text-slate-300 leading-relaxed font-sans">{translateText(slot2.uniqueness, currentLang, true)}</p>
                  </div>
                </div>
              )}
        ...
        <div className="p-3.5 md:p-4 border-t border-white/5 bg-slate-950/40 text-center font-mono text-[8px] md:text-[9px] text-slate-500 flex flex-col md:flex-row justify-between items-center gap-2 shrink-0">
          <span>{text.footerAssistant}</span>
          <span className="text-slate-650">{text.footerDisclaimer}</span>
        </div>
